...
GM_ERRCODE hard_ware_timer_proc(void)
{
	...
	if (false == hard_ware_is_device_w())
	{
		//检查电源电压过高报警
...
static ConfigDeviceTypeEnum battery_devtypes[] = 
{
    ...
    DEVICE_W1,//无线，录音，无WIFI
    DEVICE_W5,//无线，录音，WIFI
    DEVICE_W7,//无线，录音，WIFI
};
...
{				
	...
	if (hard_ware_has_battery(s_hard_ware.dev_type) == false)
	{
		return false;
	}
	...
	{
		if (s_hard_ware.battery_voltage < 7.0f)
		{
			return false;
		}
		else
...
u8 hard_ware_get_internal_battery(float battery)
{
	...
	if (dev_type_id == DEVICE_W1)
	{
		battary_level = s_1_2ah_battary_level;
	}
	else if (dev_type_id == DEVICE_W5)
	{
...
void hard_ware_internal_battery_percent(void)
{
	...
	if (s_hard_ware.battery_voltage_percent_record.false_state_hold_seconds > 180 || s_hard_ware.battery_voltage_percent_record.true_state_hold_seconds > 60)
	{
		s_hard_ware.battery_voltage_percent = battery_percent;
		s_hard_ware.battery_voltage_percent_record.false_state_hold_seconds = 0;
		s_hard_ware.battery_voltage_percent_record.true_state_hold_seconds = 0;
...
static void hard_ware_check_vcdt_voltage(void)
{
	...
	vcdt_voltage = (realtime_voltage * 2.8f * 330.0f / 39.0f) / 1024.0f;
