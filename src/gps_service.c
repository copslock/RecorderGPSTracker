...
static GM_ERRCODE protocol_pack_wifi_msg(u8 *pdata, u16 *idx, u16 len)
{
    ...
    {
    case PROTOCOL_GOOME:
        protocol_goome_pack_wifi_msg(pdata, idx, len);  //47 bytes
        break;
    case PROTOCOL_CONCOX:
    case PROTOCOL_JT808:
    default:
...
static GM_ERRCODE protocol_pack_recoder_response_msg(bool start, u8 *pdata, u16 *idx, u16 len)
{
    switch(config_service_get_app_protocol())
    {
    case PROTOCOL_GOOME:
        protocol_goome_pack_recoder_response_msg(start, pdata, idx, len);  //47 bytes
        break;
    case PROTOCOL_CONCOX:
    case PROTOCOL_JT808:
    default:
...
GM_ERRCODE gps_service_push_recoder_response_state(bool start)
{
	u8 buff[HIS_FILE_FRAME_SIZE];
	u16 len = sizeof(buff);
	u16 idx = 0;

	s_gps_socket_extend.recoder_send_time = util_clock();
	
	if(GM_SUCCESS != protocol_pack_recoder_response_msg(start, buff, &idx, len))
	{
		return GM_SYSTEM_ERROR;
	}
	...
	if(SOCKET_STATUS_WORK == s_gps_socket.status)
	{
		LOG(DEBUG,"clock(%d) gps_service_push_recoder_response_state msglen(%d)", util_clock(), idx);
		if(GM_SUCCESS == gps_service_cache_send(buff, idx))
		{
			return GM_SUCCESS;
		}
...
static GM_ERRCODE protocol_pack_recoder_file_msg(void *arg, u8 *pdata, u16 *idx, u16 len)
{
    switch(config_service_get_app_protocol())
    {
    case PROTOCOL_GOOME:
        protocol_goome_pack_recoder_file_msg(arg, pdata, idx, len);
        break;
    case PROTOCOL_CONCOX:
    case PROTOCOL_JT808:
    default:
...
GM_ERRCODE gps_service_send_one_recoder_file_pack(void *arg)
{
    u8 *buff;
    //u16 len = 0;
    u16 idx = 0;

    LOG(DEBUG,"clock(%d) gps_service_send_one_recoder_file_pack.",util_clock());
    if(s_gps_socket.status != SOCKET_STATUS_LOGIN && s_gps_socket.status != SOCKET_STATUS_WORK)
    {
        LOG(WARN,"clock(%d) gps_service_send_one_recoder_file_pack socket->status(%s) error.", util_clock(), gm_socket_status_string((SocketStatus)s_gps_socket.status));
        return GM_PARAM_ERROR;
    }
    ...
    if (!buff)
    {
        LOG(WARN,"clock(%d) gps_service_send_one_recoder_file_pack GM_MemoryAlloc fail.", util_clock());
        return GM_SYSTEM_ERROR;
    }

    GM_memset(buff, 0x00, RECD_DATA_MAX_LEN);

    if(GM_SUCCESS != protocol_pack_recoder_file_msg(arg, buff, &idx, RECD_DATA_MAX_LEN))
	{
		GM_MemoryFree(buff);
		buff = NULL;
		return GM_SYSTEM_ERROR;
    ...
    if(SOCKET_STATUS_WORK == s_gps_socket.status)
    {
        LOG(DEBUG,"clock(%d) gps_service_send_one_recoder_file_pack msglen(%d)", util_clock(), idx);
        if(GM_SUCCESS == gm_socket_send(&s_gps_socket, buff, idx))
        {
            s_gps_socket.send_time = util_clock();
            GM_MemoryFree(buff);
        ...
        {
            system_state_set_gpss_reboot_reason("gm_socket_send one_recoder_file");
            gps_service_destroy_gprs();
        }*/
    }
