...
static void protocol_goome_pack_status(U8* pdata, u16 *idx, u16 len)
{
	...
	//是否开启录音
    if (hard_ware_is_device_w() && system_state_get_recoder())
    {
    	SET_BIT12(data);
    }
...
{
    protocol_goome_pack_head(pdata, idx, len);  //7 bytes
    protocol_goome_pack_wifis(pdata, idx, len);  //47 bytes    
    protocol_goome_pack_id_len(pdata, PROTOCCOL_GOOME_WIFI, *idx);
}



static void protocol_goome_pack_recoder_state(U8* pdata, u16 *idx, u16 len)
{
    if((*idx) + 1 > len)
    {
        LOG(WARN,"clock(%d) protocol_goome_pack_recoder_state assert(len(%d)) failed.", util_clock(), len);
        return;
    }

	pdata[(*idx)++] = get_recoder_response_state();
}




void protocol_goome_pack_recoder_response_msg(bool start, U8* pdata, u16 *idx, u16 len)
{
    protocol_goome_pack_head(pdata, idx, len);  //7 bytes
    protocol_goome_pack_recoder_state(pdata, idx, len);  //1 bytes    
    if (start)
    {
...
static void protocol_goome_pack_recoder_data(void *arg, U8* pdata, u16 *idx, u16 len)
{
	RecoderSendRegStruct *op = arg;

	if (((*idx) + op->pack_len + 8) > len)
	{
		LOG(WARN, "protocol_goome_pack_recoder_data assert(len(%d/%d)) failed.", util_clock(), op->pack_len, len);
		return;
	}
...
void protocol_goome_pack_recoder_file_msg(void *arg, U8* pdata, u16 *idx, u16 len)
{
	protocol_goome_pack_head(pdata, idx, len);  //7 bytes
    protocol_goome_pack_recoder_data(arg, pdata, idx, len);  //max 908 bytes    
