...
extern void write_mtk255_config(u8 on,u16 x,u16 y,u16 z,u16 u);
	...
	switch (cmd_id)
	{
		...
		case CMD_STATUS:
		{
			...
			if (para_num == 1)
			{
				...
				if (1 == lang && COMMAND_GPRS == from)
				{
					rsp_len += GM_snprintf((char*)p_rsp, CMD_MAX_LEN, "GSM信号强度:%d,GPRS状态:%d(0:掉线,1:上线),GPS:%s,电池电压%.1fV,运动传感器:%s,设防模式:%d(0:自动,1:手动),设防状态:%d(0:撤防,1:设防)",
								gsm_get_csq(),
								gprs_is_ok(),
					...
					{
						rsp_len += GM_snprintf((char*)&p_rsp[rsp_len], CMD_MAX_LEN, ",电源电压:%.1fV,ACC:%d,油电状态:%d(0:未断开,1:已断开)",
								power_voltage,
								acc_state,
				...
				{
					rsp_len += GM_snprintf((char*)p_rsp, CMD_MAX_LEN, "GSM:%d,GPRS:%d,GPS:%s,battery:%.1fV,g_sensor:%s,defence mode:%d,defence state:%d",
								gsm_get_csq(),
								gprs_is_ok(),
								get_fixstate_str(lang, from, gps_get_state()),
								battery_voltage,
					...
					if (!hard_ware_is_device_w())
					{
						rsp_len += GM_snprintf((char*)&p_rsp[rsp_len], CMD_MAX_LEN, ",voltage:%.1fV,ACC:%d,relay:%d",
								power_voltage,
								acc_state,
								system_state_get_device_relay_state());
					}
		...
		case CMD_GPS_LPM:
		{
			...
			u16 z;
			u16 u;
			para_num = command_scan((char*)p_cmd_content, "s;iiiii", cmd_name,&mode,&x,&y,&z,&u);
			if (para_num == 6)
			{
				write_mtk255_config(mode,x,y,z,u);
			}
			GM_memcpy(p_rsp, set_success_rsp(from), CMD_MAX_LEN);
		}
		break;
		...
		case CMD_NMEAINTERVAL:
		{
			u8 nmea_interval;
			para_num = command_scan((char*)p_cmd_content, "s;i", cmd_name,&nmea_interval);
			if (para_num == 1)
			{
				config_service_get(CFG_NMEATIME, TYPE_BYTE, &nmea_interval, sizeof(nmea_interval));
				if (1 == lang && COMMAND_GPRS == from)
				{
					GM_snprintf((char*)p_rsp, CMD_MAX_LEN, "NMEA间隔:%d",nmea_interval);
				}
				else
				{
					GM_snprintf((char*)p_rsp, CMD_MAX_LEN, "nmea interval:%d",nmea_interval);
				}
			}
			else if(para_num == 2)
			{
				config_service_set(CFG_NMEATIME, TYPE_BYTE, &nmea_interval, sizeof(nmea_interval));
				if(GM_SUCCESS != config_service_save_to_local())
				{	
					GM_memcpy(p_rsp, set_fail_rsp(from), CMD_MAX_LEN);
				}
